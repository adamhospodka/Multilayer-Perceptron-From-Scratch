...
class Adam: public Optimizer<NT> {
...
public:
    ...
    Adam<NT>(NT learning_rate, NT beta1, NT beta2, const char* name="Adam"): Optimizer<NT>(name, learning_rate),
    beta1(beta1), beta2(beta2),
    //epsilon(epsilon),
    //t(0),
    beta1_pow_t(beta1),
    beta2_pow_t(beta2) {
        cout << this->name << ": *breaks ouf of the coffin*" << endl;
    }
...
public:
    void prepare_memory() override {
        cout << this->name << ": *grunts*" << endl;
        cout << this->name << ": \"Let me just allocate memory for your parameters real quick...\"" << endl;
        ...
        for (const Mx* m_ref: this->parameter_matrices_references) {
            ...
            //cout << *m_ref << endl << *m_ref << endl;
            Mcors.emplace_back(*m_ref);
            Vcors.emplace_back(*m_ref);
        }
        cout << this->name << ": \"I'm ready to optimize, homie.\"" << endl;
